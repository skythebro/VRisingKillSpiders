...
namespace SpiderKiller
{
	public static class Settings
	{
		...
		internal static void Initialize(ConfigFile config)
		{
			...
			ENABLE_EXTRA_CULL_REWARD = config.Bind<bool>("Server", "enableExtraCullReward", false, "(not working for now)Enables the extra cull reward of silkworms");
			...
			SILKWORM_GIVE_AMOUNT = config.Bind<int>("Server", "silkwormGiveAmount", 1, "(not working for now) Amount of silkworms to give when culling 'cullAmountThreshold' worth of spiders");
		}
	}
}
